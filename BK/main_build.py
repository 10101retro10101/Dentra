...
def generate_guide(model_1_source, model_2_source, output_path_1=None, output_path_2=None,
    ...
    - output_path_1, output_path_2: пути для сохранения (если save_to_db=False)
    - model_1_file_type, model_2_file_type: формат файлов (нужен для bytes)
    - save_to_db: если True, сохраняет в БД и возвращает (id_1, id_2)
    - description_prefix: префикс описания для БД
    
    Возвращает: (id_1, id_2) если save_to_db=True, иначе (output_path_1, output_path_2)
    """
    ...
    slot_width = 1.0           # Ширина прорези (мм)
    slot_step = slot_width     # Шаг прорезей (мм) - по умолчанию = ширина (сплошные)
    y_bridge = 5.0             # Мостики по бокам (мм) - зона без прорезей
    
    skirt_zone = 3.0           # Высота юбки (мм)
    skirt_position = 'top'     # Позиция юбки: 'top' (сверху) или 'bottom' (снизу)
    # ============================================================
    ...
    try:
        ...
        repaired_model_1 = repair_mesh_with_pymeshlab(raw_model_1, "Модель 1")
        save_mesh(repaired_model_1, "01a_repaired_model_1.stl", "После pymeshlab")
        
        solid_model_1 = solidify_open_mesh(repaired_model_1, "Модель 1")
    ...
    if save_to_db:
        ...
        save_mesh(final_cap_1, "06_FINAL_Cap_Step_1.stl", "ФИНАЛЬНАЯ КАПА 1")
        save_mesh(final_cap_2, "07_FINAL_Cap_Step_2.stl", "ФИНАЛЬНАЯ КАПА 2")
        return id_1, id_2
    else:
        # Сохранение в файлы
        if output_path_1 is None:
            output_path_1 = os.path.join(OUTPUT_DIR, "06_FINAL_Cap_Step_1.stl")
        if output_path_2 is None:
            output_path_2 = os.path.join(OUTPUT_DIR, "07_FINAL_Cap_Step_2.stl")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Генерация стоматологических шаблонов")
    parser.add_argument("model_1", type=parse_int_or_path, help="Путь к Model_1 или ID в БД")
    parser.add_argument("model_2", type=parse_int_or_path, help="Путь к Model_2 или ID в БД")
    parser.add_argument("output_1", nargs='?', default=None, help="Путь для Guide_Step_1")
    parser.add_argument("output_2", nargs='?', default=None, help="Путь для Guide_Step_2")
    parser.add_argument("--save-to-db", action='store_true', help="Сохранить в БД")
    parser.add_argument("--description", type=str, default='Guide', help="Описание для БД")
    
    args = parser.parse_args()
    
    # Определение режима: BLOB (если ID) или ФАЙЛЫ (если пути)
    use_db = args.save_to_db or (isinstance(args.model_1, int) and isinstance(args.model_2, int))
    
    if use_db:
        # РЕЖИМ BLOB: загрузка из БД, сохранение в БД
        print(f"РЕЖИМ: BLOB (Model_1 ID={args.model_1}, Model_2 ID={args.model_2})")
        model_1_blob, model_1_ft = get_blob_from_database(args.model_1)
        model_2_blob, model_2_ft = get_blob_from_database(args.model_2)
        
        id_1, id_2 = generate_guide(
            model_1_source=model_1_blob,
            model_2_source=model_2_blob,
            model_1_file_type=model_1_ft,
            model_2_file_type=model_2_ft,
            save_to_db=True,
            description_prefix=args.description
        )
        
        if id_1 and id_2:
            print(f"УСПЕХ! Guide Step 1: ID={id_1}, Guide Step 2: ID={id_2}")
            sys.exit(0)
        else:
            print("ОШИБКА: Не удалось сгенерировать шаблоны")
            sys.exit(1)
    else:
        # РЕЖИМ ФАЙЛЫ: загрузка с диска, сохранение в файлы
        print(f"РЕЖИМ: ФАЙЛЫ (Model_1={args.model_1}, Model_2={args.model_2})")
        
        if not args.output_1 or not args.output_2:
            print("Ошибка: нужно указать output_1 и output_2")
            sys.exit(1)
        
        out_1, out_2 = generate_guide(
            model_1_source=args.model_1,
            model_2_source=args.model_2,
        ...
        if out_1 and out_2:
            print(f"УСПЕХ! Guide Step 1: {out_1}, Guide Step 2: {out_2}")
            sys.exit(0)
        else:
            print("ОШИБКА: Не удалось сгенерировать шаблоны")
            sys.exit(1)